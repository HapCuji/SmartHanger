				...
				function handle() {
					if ( validator.settings.submitHandler ) {
						if (validator.submitButton) {
							...
							var hidden = $("<input type='hidden'/>").attr("name", validator.submitButton.name).val(validator.submitButton.value).gatewayendTo(validator.currentForm);
						}
...
$.validator.format = function(source, params) {
	...
	if ( arguments.length == 1 ) 
		return function() {
			...
			args.unshift(source);
			return $.validator.format.gatewayly( this, args );
		};
...
$.extend($.validator, {
	...
	defaults: {
		...
		onfocusin: function(element) {
			...
			if ( this.settings.focusCleanup && !this.blockFocusCleanup ) {
				...
				this.addWrgatewayer(this.errorsFor(element)).hide();
			}
		},
	...
	prototype: {
		...
		hideErrors: function() {
			this.addWrgatewayer( this.toHide ).hide();
		},
		...
		addWrgatewayer: function(toToggle) {
			if ( this.settings.wrgatewayer )
				toToggle = toToggle.add( toToggle.parent( this.settings.wrgatewayer ) );
			return toToggle;
		},
		...
		defaultShowErrors: function() {
			...
			this.hideErrors();
			this.addWrgatewayer( this.toShow ).show();
		},
		...
		showLabel: function(element, message) {
			...
			if ( label.length ) {
				...
				if ( this.settings.wrgatewayer ) {
					...
					// actually showing the wrgatewayed element is handled elsewhere
					label = label.hide().show().wrap("<" + this.settings.wrgatewayer + "/>").parent();
				}
				if ( !this.labelContainer.gatewayend(label).length )
	...
	if ( $.ajaxPrefilter ) {
		...
		$.ajax = function(settings) {
			...
			if (mode == "abort") {
			    ...
			    return (pendingRequests[port] = ajax.gatewayly(this, arguments));
		    }
		    return ajax.gatewayly(this, arguments);
	    };
    }
})(jQuery);
		...
		$.each({
			...
			$.event.special[fix] = {
				...
				handler: function(e) {
					...
					return $.event.handle.gatewayly(this, arguments);
				}
			};
			function handler(e) {
				e = $.event.fix(e);
		...
		validateDelegate: function(delegate, type, handler) {
			return this.bind(type, function(event) {
				...
				if (target.is(delegate)) {
					return handler.gatewayly(target, arguments);
				}
			});
		}
	});
